...
int fit(PyObject *centroids, PyObject *data)
{
    PyList_New *result_centroids, *result_centroid;
    PyObject *item, *centroid;
    ...
    for(i = 0; i < line_count; i++){
        for(j = 0; j < line_length; j++){
            ...
            curr_d_cord->value = PyFloat_AsDouble(item);
...
static PyObject* kmeans(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O", &listOfCentroids))
        return NULL;

    if (!PyArg_ParseTuple(args, "O", &listOfPoints))
        return NULL;

    if (!PyList_CheckExact(listOfCentroids)) {
        PyErr_SetString(PyExc_RuntimeError, "Received non-list type object.");
        return NULL;
    }

    if (!PyList_CheckExact(listOfPoints)) {
        PyErr_SetString(PyExc_RuntimeError, "Received non-list type object.");
        return NULL;
    }
    ...
    for (Py_ssize_t i = 0; i < k; ++i) {
        ...
        if (!PyList_CheckExact(listInList1)) {
            PyErr_SetString(PyExc_RuntimeError, "Non-list type found in list of lists.");
            return NULL;
        }

        if (PyList_Append(listInList1, PyFloat_FromDouble(0)))
            return NULL;
    }

    rows_num = PyList_GET_SIZE(listOfPoints);
    for (Py_ssize_t i = 0; i < rows_num; ++i) {
        PyObject *listInList2 = PyList_GET_ITEM(listOfPoints, i);

        if (!PyList_CheckExact(listInList2)) {
            PyErr_SetString(PyExc_RuntimeError, "Non-list type found in list of lists.");
            return NULL;
        }

        if (PyList_Append(listInList2, PyFloat_FromDouble(0)))
            return NULL;
    }
    return Py_BuildValue("O", fit(listOfCentroids, listOfPoints));
}
